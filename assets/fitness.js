Bundle = {
  ...
  Utils: {
    ...
    updateBundleQuantity: function($item, increment) {
        ...
        if (new_bundle_qty <= 1) {
            ...
            $('.bundle-images').children('div').each(function(i) { 
                if ($(this).hasClass('bundle_placeholder') && bundle_image_qty == 0) {
                  	$(this).remove();
                    $('.bundle-images').append('<div class="bundle_placeholder grid__item small--one-quarter medium-up--one-quarter"> <img src="' + $bundle_placeholder_last + '"><h6>Select Box</h6></div>')
                    bundle_image_qty = 1;
                }
            });
        } else {
      ...
      if (new_bundle_qty <= 3) {
          $('#bundle-add-to-cart .cart-text').text('Minimum 4 Packs');
		  $('#bundle-add-to-cart .bundle-price').text('');
          $('.fitness_inside_section .cart-text').text('Minimum 4 Packs');
		  $('.fitness_inside_section .bundle-price').text('');
      } else {
       	  $('#bundle-add-to-cart .cart-text').text('- Add to Cart');
          $('.fitness_inside_section .cart-text').text('- Add to Cart');
      }
    ...
    updatePrice: function() {
      ...
      $bundle.find('.bundle-item').each(function() {
        ...
        if (qty > 0) {
          ...
          var variant_id_not_sub = $(this).attr('data-variant-id');
          var variant_price_not_sub = product.variant_to_price[variant_id];
          ...
          not_sub_price += qty * (Number(variant_price_not_sub) / 100);
      ...
      var formatted_price_not_sub = (not_sub_price).toLocaleString('en-US', {
        style: 'currency',
        currency: 'USD',
      });
      if (price != 0) {
        if (price != not_sub_price) {
        	$details.find('.bundle-price').html(formatted_price);  
        	$details.find('.bundle-orig-price').html(formatted_price_not_sub);  
          	$('.fitness_inside_section .bundle-price').html(formatted_price);  
          	$('.fitness_inside_section .bundle-orig-price').html(formatted_price_not_sub);  
          	$details.find('.bundle-orig-price').show();
          	$('.fitness_inside_section .bundle-orig-price').show();
        } else {
        	$details.find('.bundle-price').html(formatted_price);  
          	$('.fitness_inside_section .bundle-price').html(formatted_price);  
          	$details.find('.bundle-orig-price').hide();
          	$('.fitness_inside_section .bundle-orig-price').hide();
        }
      }
  ...
  Cart: {
    ...
    init: function() {
      ...
      $( document ).ready(function() {
        ...
        $('select.rc_select__frequency  option[value="4"]').attr("selected",true);

        setTimeout(function() 
        {
          $('.rc_radio.rc_radio__autodeliver').click();
          $('.bundle-grid').children('.bundle_product').each(function(i) { 
